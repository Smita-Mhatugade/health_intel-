...
def test_reverse_geocoding_pune():
    """TEST 13.1: Reverse geocoding for Pune."""
    try:
        # Pune coordinates
        resp = requests.get(f"{BASE_URL}/api/reverse-geocode?lat=18.5204&lon=73.8567", timeout=15)
        data = resp.json()
        if resp.status_code == 200:
            city = data.get("city", "")
            source = data.get("source", "unknown")
            if "Pune" in city:
                log("GET /api/reverse-geocode", PASS, f"Correctly identified Pune (source: {source})")
            else:
                log("GET /api/reverse-geocode", FAIL, f"Expected Pune, got {city} (source: {source})")
        else:
            log("GET /api/reverse-geocode", FAIL, f"Status {resp.status_code}", data)
    except Exception as e:
        log("GET /api/reverse-geocode", FAIL, str(e))


def test_geoapify_connection():
    """TEST 13.2: Verify Geoapify connection by checking source of reverse geocode."""
    try:
        # Request should ideally be served from geoapify if key is valid
        resp = requests.get(f"{BASE_URL}/api/reverse-geocode?lat=18.5204&lon=73.8567", timeout=15)
        data = resp.json()
        if resp.status_code == 200:
            source = data.get("source", "")
            if "geoapify" in source:
                log("Geoapify Connection", PASS, "API Key is valid and Geoapify is responding")
            else:
                log("Geoapify Connection", WARN, f"Falling back to {source}. Is GEOAPIFY_API_KEY set correctly?")
        else:
            log("Geoapify Connection", FAIL, f"Status {resp.status_code}", data)
    except Exception as e:
        log("Geoapify Connection", FAIL, str(e))
...
if __name__ == "__main__":
    ...
    print("\n" + "─" * 70)
    print("🏥 SECTION 3: Hospital Recommendation & Location")
    print("─" * 70)
    test_hospital_recommendation()
    test_reverse_geocoding_pune()
