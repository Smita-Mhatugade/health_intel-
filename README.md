...
- **Smart Hospital Recommendations:** Automatically detects user location and finds nearby specialized hospitals.
- **Massive Local Healthcare Database:** Integrates a robust, locally cached database of **10,000+ hospitals and clinics** across Maharashtra for instant, API-independent search results.
- **Interactive UI & Navigation:** Map integration with auto-panning, hover states, and direct "Get Directions" routing to Google Maps.
- **Modern Tech Stack:** Built with React, TypeScript, TailwindCSS, and Shadcn UI for a clean, responsive experience.
...
graph TD;
    ...
    Backend -->|1. Primary Search| LocalCSV[(Local 10k+ CSV)];
    Backend -->|2. Fallback API| Geoapify[Geoapify Maps API];
```
...
```bash
git clone https://github.com/Smita-Mhatugade/health_intel-.git
cd health_intel-
```
